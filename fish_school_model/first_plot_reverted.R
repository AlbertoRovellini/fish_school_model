...
cellSize <- c(500,250,125,62.500,31.250,15.625)
relevantFish$cellSize <- cellSize
relevantFish <- relevantFish[with(relevantFish, order(cellSize)), ]

colnames(relevantFish) <- c("Numberoffish", "Polarization", "Foodpatch", "Fishonfood", "Schoolextent",
                            "pol_se", "fp_se", "fof_se", "se_se", "Grid", "Cellsize")
meltFish <- melt(relevantFish, id.vars="Cellsize",variable.name= "variable", 
...
ggsave("First plot reverted.pdf", plotNumberFish, useDingbats=FALSE)
